...
class Mail {
    ...
    private function configureMailer() {
        ...
        $this->mailer->Username = 'bookhavenuwu@gmail.com'; // Replace with your email address
        $this->mailer->Password = 'ipjcujnxdavxkrot'; // Replace with your email password
        $this->mailer->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $this->mailer->Port = 587; // Replace with your SMTP port
        $this->mailer->setFrom('noreply@bookhaven.com', 'BookHaven Account Term');
    }

    private function send($email, $subject, $message) {
        try {
            $this->mailer->addAddress($email, 'Recipient Name');
            $this->mailer->addReplyTo('noreply@bookhaven.com', 'noReply');
